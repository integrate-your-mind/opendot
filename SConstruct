...
opts.Add(BoolVariable("production", "Set defaults to build Opendot for use in production", False))
...
opts.Add("vsproj_name", "Name of the Visual Studio solution", "opendot")
...
if env["platform"] == "osx":
    ...
    print_warning('Platform "osx" has been renamed to "macos" in Opendot 4. Building for platform "macos".')
    env["platform"] = "macos"

if env["platform"] == "iphone":
    # Deprecated alias kept for compatibility.
    print_warning('Platform "iphone" has been renamed to "ios" in Opendot 4. Building for platform "ios".')
    env["platform"] = "ios"

if env["platform"] in ["linux", "bsd", "x11"]:
    if env["platform"] == "x11":
        # Deprecated alias kept for compatibility.
        print_warning('Platform "x11" has been renamed to "linuxbsd" in Opendot 4. Building for platform "linuxbsd".')
    # Alias for convenience.
    env["platform"] = "linuxbsd"

if env["platform"] == "javascript":
    # Deprecated alias kept for compatibility.
    print_warning('Platform "javascript" has been renamed to "web" in Opendot 4. Building for platform "web".')
    env["platform"] = "web"
...
if cc_version_major == -1:
    ...
    if cc_version_major < 9:
        print_error(
            ...
            "C++17, or has bugs when compiling Opendot. Supported versions are 9 "
    ...
    # https://github.com/opendotengine/opendot/pull/94995#issuecomment-2336464574
    if cc_version_major == 16 and cc_version_minor < 11:
        print_error(
            "Detected Visual Studio 2019 version older than 16.11, which has bugs "
            "when compiling Opendot. Use a newer VS2019 version, or VS2022."
        )
        Exit(255)
    if cc_version_major == 15 and cc_version_minor < 9:
        print_error(
            "Detected Visual Studio 2017 version older than 15.9, which has bugs "
            "when compiling Opendot. Use a newer VS2017 version, or VS2019/VS2022."
        )
        Exit(255)
    if cc_version_major < 15:
        print_error(
            "Detected Visual Studio 2015 or earlier, which is unsupported in Opendot. "
...
# Disable exception handling. Opendot doesn't use exceptions anywhere, and this
